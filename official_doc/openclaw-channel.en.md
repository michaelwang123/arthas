...
```
[Arthas 配置错误] shareCode 格式无效: roomId �?key 段不能为�?
  当前值的�? [�?="(�?", �?="..."]
  设置方式: 环境变量 ARTHAS_SHARE_CODE=your-room-id:your-encryption-key
```
...
## Complete Example: AI Agent Connection

A full runnable example showing how to connect to an Arthas server, send messages, listen for replies, and disconnect safely:

```typescript
// verify-plugin.ts �?Complete AI Agent connection example
// Run with: npx tsx verify-plugin.ts

import { ArthasChannelAdapter } from '@arthas-chat/openclaw-channel';

const SERVER_URL = 'wss://arthas100-arthas-server.hf.space/ws';
const SHARE_CODE = 'your-share-code';  // Get from Arthas client after creating a room

async function main() {
  const adapter = new ArthasChannelAdapter();

  // 📚 学习要点: Register callbacks before connecting.
  // The Gateway calls onMessage �?onStatusChange �?connect in order,
  // ensuring no messages are lost after the connection is established.
  ...
  // Listen for user messages (decrypted plaintext)
  adapter.onMessage(async (msg) => {
    console.log(`[${msg.userName}] ${msg.text}`);

    // Call your AI here (OpenAI, Claude, etc.)
    const reply = `Received: "${msg.text}" �?This is an automated AI Agent reply`;

    // Send encrypted reply back to the user
    await adapter.send({
      id: crypto.randomUUID(),
      channelId: 'arthas',
      text: reply,
  ...
  // Connect to the Arthas encrypted room
  await adapter.connect({
    serverUrl: SERVER_URL,
    shareCode: SHARE_CODE,
    displayName: 'AI Assistant',
    signingEnabled: true,  // Enable Ed25519 message signing
  });

  console.log('�?AI Agent online, waiting for user messages...');

  // Graceful shutdown (zero keys on Ctrl+C)
  process.on('SIGINT', async () => {
    await adapter.disconnect();
    console.log('Disconnected, keys zeroed');
    process.exit(0);
  });
}
...
After running this script, join the same room from the Arthas Web client or CLI using the same share code, and send a message to see the AI Agent's automated reply.
