...
- [x] 0. 前置调研：验证 OpenClaw SDK API (~1h)
  - [x] 0.1 安装 @openclaw/sdk 并检查实际导出的类型
...
- [x] 1. 项目初始化与基础设施 (~2h)
  - [x] 1.1 创建 `packages/openclaw-channel/` 目录结构
...
- [x] 2. 加密引擎 (~3h)
  - [x] 2.1 实现 AES-256-GCM 加密/解密 (src/crypto.ts)
...
- [x] 3. 协议层 (~2h)
  - [x] 3.1 实现 msgpack 编解码 (src/protocol.ts)
...
- [x] 4. WebSocket 客户端 (~3h)
  - [x] 4.1 实现 Arthas WebSocket 客户端 (src/client.ts)
...
- [x] 5. OpenClaw Channel Adapter (~3h)
  - [x] 5.1 实现 Channel Adapter (src/adapter.ts)
  ...
  - [x] 5.2 实现 Plugin 入口 (src/index.ts)
