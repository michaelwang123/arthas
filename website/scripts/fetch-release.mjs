 ...
 *
 * 职责：
 * - 从 GitHub API 获取 punkcanyang/arthas 仓库的最新 Release 信息
 * - 提取版本号、发布时间、各平台二进制资产的下载链接
 * - 将结果写入 src/data/release.json 供 Astro 页面组件读取
 * - API 不可达时写入 fallback 数据并输出警告日志
 *
 * 与其他模块的关系：
 * - package.json build script 在 astro build 前执行本脚本
 ...
 * - 未认证请求：60 次/小时（按 IP）
 * - 认证请求（带 token）：5000 次/小时
 * - CI 环境通常有 GITHUB_TOKEN，但本脚本不强制要求
 * - 如果超出限制，API 返回 403，脚本会 fallback 到硬编码数据
 */
const GITHUB_API_URL = 'https://api.github.com/repos/punkcanyang/arthas/releases/latest';

/**
 * Fallback 版本号 — 当 API 不可达时使用
 * 应在每次正式发版后手动更新此值
 */
...
/**
 * 从 GitHub API 获取最新 Release 数据。
 *
 * 📚 学习要点: fetch API 在 Node.js 中的使用
 * - Node.js 18+ 内置全局 fetch（无需安装 node-fetch）
 * - 使用 AbortController 实现请求超时，避免构建无限等待
...
async function fetchLatestRelease() {
  ...
  try {
    ...
    // CI 环境（GitHub Actions）自动注入 GITHUB_TOKEN
    // 本地开发时通常不需要（60 次/小时足够）
...
/**
 * 将 GitHub API 返回的 Release 数据转换为精简的站点数据格式。
 *
 * 📚 学习要点: 数据转换层的意义
 * - GitHub API 返回大量字段（作者信息、Markdown body 等），页面只需要一小部分
 * - 精简数据减少 JSON 文件体积，加快页面加载
 * - 统一的数据结构让前端组件不依赖 GitHub API 的具体格式
 *
...
/**
 * 生成 fallback 数据 — 当 GitHub API 不可达时使用。
 *
 * 📚 学习要点: 优雅降级（Graceful Degradation）
 * - 构建不应因外部 API 不可达而失败
 * - fallback 数据标记 `fallback: true`，前端可据此显示提示
 * - 空 assets 数组意味着下载页面可以显示"请访问 GitHub Releases"的引导
 *
...
/**
 * 确保 src/data/ 目录存在。
 * Astro 项目默认不包含 data 目录，首次运行时需要创建。
 */
...
/**
 * 主函数 — 获取 Release 数据并写入 JSON 文件。
 *
 * 流程：
 * 1. 确保输出目录存在
 * 2. 尝试从 GitHub API 获取最新 Release
 * 3. 成功 → 转换并写入精简数据
...
// ─── 模块导出（供测试使用） ────────────────────────────────────────────────────
// 📚 学习要点: ESM 条件执行模式
// 通过比较 import.meta.url 和 process.argv[1] 判断脚本是被直接执行还是被 import。
// 直接执行时运行 main()，被 import 时仅导出函数供测试调用。
