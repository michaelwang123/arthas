	...
	rapid.Check(t, func(t *rapid.T) {
		...
		for i := 0; i < numPending; i++ {
			...
			if isExpired {
				...
				// Created recently — within the timeout window (0 to timeout-1s ago).
...
// TestHandleMatchKeyRelay_HappyPath verifies the full key relay → room creation → MatchFound flow.
...
func TestHandleMatchKeyRelay_NotClientA(t *testing.T) {
	...
	// Client B tries to relay — should be rejected.
...
func TestHandleReport_InvalidReason(t *testing.T) {
	...
	// Should silently ignore — no messages to reporter.
	reporter.mu.Lock()
	msgs := reporter.sent
	reporter.mu.Unlock()
	if len(msgs) != 0 {
...
func TestHandleReport_NotInRoom(t *testing.T) {
	...
	// Should silently ignore — no messages.
	reporter.mu.Lock()
	msgs := reporter.sent
	reporter.mu.Unlock()
	if len(msgs) != 0 {
