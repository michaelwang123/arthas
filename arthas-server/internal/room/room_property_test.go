// room_property_test.go �?Room 过期属性测�?
//
// 本文件使�?pgregory.net/rapid 进行属性测试（Property-Based Testing），
// 验证 Room.IsExpired、Room.GetExpiresAt、RoomManager.GetExpiredRooms 的正确性�?
//
// 📚 学习要点: 属性测�?vs 示例测试
// 属性测试不预设具体输入/输出，而是验证「对于所有合法输入，某个属性恒成立」�?
// 例如：对于任�?expiry > 0，expiresAt 应等�?now + expiry（在合法范围内）�?
...
//
// 属性定义：
// 给定合法�?expiry�? �?MaxExpiryDuration），expiresAt 应等�?now + expiry�?
// 验证 GetExpiresAt() 返回值与创建时传入的 expiresAt 一致，
// �?IsExpired �?expiresAt 之前返回 false，之后返�?true�?
//
// **Validates: Requirements 5.1, 5.2**

// TestProperty3_ExpiresAtComputation_PositiveExpiry 验证正数 expiry 产生正确�?expiresAt�?
//
// 测试策略：模�?hub.go 的计算逻辑：expiresAt = now + expiry（当 expiry > 0）�?
// 验证 NewRoom 创建�?GetExpiresAt() 返回正确值�?
// 验证 IsExpired �?expiresAt 之前返回 false，之后返�?true�?
func TestProperty3_ExpiresAtComputation_PositiveExpiry(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		// 生成合法�?expiry�? �?MaxExpiryDuration�?
		expiry := rapid.Int64Range(1, MaxExpiryDuration).Draw(t, "expiry")

		// 模拟当前时间（Unix 秒）
		now := rapid.Int64Range(1_000_000_000, 2_000_000_000).Draw(t, "now")

		// 计算 expiresAt（模�?handleCreateRoom 的逻辑�?
		expiresAt := now + expiry

		// 创建房间
		room := NewRoom("test-room-id-21chars", "", 0, expiresAt, 0)
...
// TestProperty3_ExpiresAtComputation_ZeroExpiry 验证 expiry=0 �?expiresAt=0�?
// 即永不过期的房间，IsExpired 对任何时间点都返�?false�?
func TestProperty3_ExpiresAtComputation_ZeroExpiry(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		// expiresAt = 0 表示永不过期
		room := NewRoom("test-room-id-21chars", "", 0, 0, 0)
...
// TestProperty3_ExpiresAtComputation_BoundaryExact 验证 now == expiresAt �?
// IsExpired 返回 false（严格大于才过期，即 now > expiresAt 才为 true）�?
//
// 测试策略：验证边界条件的精确行为�?
// IsExpired 使用 now > expiresAt（严格大于），所�?now == expiresAt 时不过期�?
...
//
// 属性定义：
// 对于任意房间集合，GetExpiredRooms(T) 应返回所�?expiresAt > 0 && T > expiresAt 的房间�?
// 满足条件的房间必须在列表中，不满足条件的房间不应在列表中�?
//
// **Validates: Requirements 6.2, 6.5**

// TestProperty4_ExpiryCheckerCorrectness 验证 GetExpiredRooms 的正确�?完整性�?
//
// 测试策略：创建多个房间（混合过期/未过�?永不过期），
...
func TestProperty4_ExpiryCheckerCorrectness(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		for i := 0; i < roomCount; i++ {
			...
			// 生成 expiresAt�?=永不过期�?scanTime=已过期，>=scanTime=未过�?
			...
			rm.CreateRoom(roomID, "", 0, expiresAt, 0)
		}
...
// 调用 RemoveRoom �?RoomManager 中不再包含该房间�?
...
func TestProperty8_EmptyRoomDestructionInvariant(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// 模拟 hub.go 的逻辑：成员为 0 时调�?RemoveRoom
		rm.RemoveRoom(roomID)

		// 验证房间已从 RoomManager 中移�?
		if rm.GetRoom(roomID) != nil {
...
//
// 属性定义：
// - �?expiry < 0，sanitizeExpiry 应返�?expiresAt=0（视为永不过期）�?
// - �?expiry > MaxExpiryDuration，应截断�?MaxExpiryDuration，expiresAt = now + MaxExpiryDuration�?
// - �?0 <= expiry <= MaxExpiryDuration，expiresAt 应在合法范围内�?
//
// 测试策略：模�?hub.go 中的 expiry 清洗逻辑，验证边界处理正确�?
// 这些测试验证防御性输入清洗（Defensive Input Sanitization）的正确性�?
// 客户端可能发送任�?expiry 值（bug、恶意行为），服务器必须做边界检查�?
//
...
// 规则�?
// - expiry < 0 �?返回 0（永不过期）
// - expiry > MaxExpiryDuration �?截断�?MaxExpiryDuration
...
// TestProperty9_ExpirySanitization_ValidRange 验证合法范围内的 expiry 产生正确�?expiresAt�?
