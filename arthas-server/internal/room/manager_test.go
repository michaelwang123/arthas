...
func TestCreateRoom(t *testing.T) {
	...
	room := rm.CreateRoom("room-abc123")
	if room == nil {
...
func TestCreateRoom_DuplicateReturnsExisting(t *testing.T) {
	...
	room1 := rm.CreateRoom("room-1")
	room2 := rm.CreateRoom("room-1")
...
func TestConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(id int) {
			defer wg.Done()
			rm.CreateRoom(fmt.Sprintf("room-%d", id))
		}(i)
	}
	wg.Wait()
