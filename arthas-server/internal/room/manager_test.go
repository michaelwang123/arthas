...
func TestCreateRoom(t *testing.T) {
	...
	room := rm.CreateRoom("room-abc123", "", 0)
	if room == nil {
...
func TestCreateRoom_DuplicateReturnsExisting(t *testing.T) {
	...
	room1 := rm.CreateRoom("room-1", "", 0)
	room2 := rm.CreateRoom("room-1", "", 0)
...
func TestRemoveRoom(t *testing.T) {
	...
	rm.CreateRoom("room-to-remove", "", 0)
	if rm.RoomCount() != 1 {
...
func TestConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(id int) {
			defer wg.Done()
			rm.CreateRoom(fmt.Sprintf("room-%d", id), "", 0)
		}(i)
	}
	wg.Wait()
